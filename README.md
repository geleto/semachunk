...
3.  **Optimized Merging Algorithm**: Uses a more optimal algorithm than the original. Instead of linear merges, it selects the best chunk pairs from the text to merge first (global priority). It also recalculates embeddings in batches for significantly better speed, especially with external embedding models.
...
| `returnTokenLength` | `true` | Include token length in the output |
